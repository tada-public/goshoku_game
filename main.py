    ...
    def draw_board(self,cnt,stage):
        ...
        for i in range(card_num):
            if self.card_rect[i] is not None and i != self.draggingItemIndex:
                self.screen.blit(self.rotated_img[i], self.card_rect[i].topleft)
                ...
                if event.type == pygame.QUIT:
                    ...
                    if (clicked in range(BOARD_SIZE[0]) and not game.double_mode_flag) \
                        ...
                        if game.cpu_mode_flag:
                         ...
                         game.color=None
                         game.color_2=None
                    elif game.double_rect_on.collidepoint(px, py) or game.text_double_rect_off.collidepoint(px, py):
